...
def add_to_wishlist(request, id):
    ...
    if product.users_wishlist.filter(id=request.user.id).exists():
        ...
        messages.success(request, product.title + " has been removied from your WishList")
    else:
...
def account_activete(request, uidb64, token):
    try:
        ############################
    ...
    if user is not None and account_activation_token.check_token(user, token):
        ...
        user.save()
        login(request, user)
        return render("account:dashboard")
    else:
...
def edit_address(request,id):
    if request.method == "POST":
        ...
        address_from = UserAddressForm(instance=address,data=request.POST)
        if address_from.is_valid():
            address_from.save()
            return HttpResponseRedirect(reverse("account:addresses"))
    else:
        address = Address.objects.get(pk=id, customer=request.user)
        address_from = UserAddressForm(instance=address)
    return render(request,"account/dashboard/edit_addresses.html",{"form": address_from} )
...
def set_default(request, id):
    ...
    if "selivery_address" in previous_url:
