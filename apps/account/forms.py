...
class PwdResetForm(PasswordResetForm):
    ...
    def clean_email(self):
        ...
        if not u:
            raise forms.ValidationError("Unfortunatly we can not find that email addres")
        return email
...
class UserEditForm(forms.ModelForm):
    email = forms.EmailField(
        label='Account email (can not be changed)', max_length=50, min_length=4,widget=forms.TextInput(
